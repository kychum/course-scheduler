...
public class Assignment {
	...
	public HashSet<Slot> getLabsMinViolations() {
		HashSet<Slot> out = assignments.keySet().stream()
			.filter( slot -> slot.getMinAssign() > assignments.get(slot).size() && instance.getLabSlotsHash().contains( slot ) )
			.collect( Collectors.toCollection( HashSet::new ) );
		return out;
	}

	public HashSet<Slot> getCourseMinViolations() {
		HashSet<Slot> out = assignments.keySet().stream()
			.filter( slot -> slot.getMinAssign() > assignments.get(slot).size() && instance.getCourseSlotsHash().contains( slot ) )
			.collect( Collectors.toCollection( HashSet::new ) );
		return out;
	}
